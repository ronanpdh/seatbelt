...
from seatbelt.ledger.events import GENESIS_HASH, Actor, ActorType, Event, Kind
...
def test_hash_is_deterministic() -> None:
    e = Event(run_id="r", seq=0, kind=Kind.RUN_START, actor=AGENT, prev_hash=GENESIS_HASH)
...
def test_editing_any_attr_breaks_the_chain(tampered: str) -> None:
    ledger_events = [
        Event(run_id="r", seq=0, kind=Kind.RUN_START, actor=AGENT, prev_hash=GENESIS_HASH).sealed()
    ]
    original = Event(
        run_id="r",
        seq=1,
